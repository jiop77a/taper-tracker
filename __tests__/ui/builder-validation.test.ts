...
describe("Builder UI Input Validation", () => {
  ...
  describe("Dosing bounds validation", () => {
    ...
    test("should accept current dose of exactly 2.0 pills", () => {
      ...
      expect(errors).not.toContain(
        "Current dose cannot exceed 2.0 pills per day"
      );
    });
  ...
  describe("Dose relationship validation", () => {
    ...
    test("should reject reduction smaller than 0.25 pills", () => {
      ...
      expect(errors).toContain(
        "Reduction must be at least 0.25 pills (quarter pill)"
      );
    });
  ...
  describe("Boundary value testing", () => {
    test("should test exact boundary values", () => {
      ...
      expect(validateInputs("2.001", "1.0")).toContain(
        "Current dose cannot exceed 2.0 pills per day"
      );

      // Test minimum reduction
      expect(validateInputs("1.0", "0.75")).toHaveLength(0); // Exactly 0.25
      expect(validateInputs("1.0", "0.751")).toContain(
        "Reduction must be at least 0.25 pills (quarter pill)"
      );

      // Test zero goal dose
      expect(validateInputs("0.5", "0")).toHaveLength(0);
      expect(validateInputs("0.5", "-0.001")).toContain(
        "Goal dose cannot be negative"
      );
    });
  });

  describe("Manual constraint handling", () => {
    test("should handle empty manual constraints without validation errors", () => {
      // This test simulates the scenario where user switches to manual mode
      // but doesn't enter any values - should not cause constraint validation errors
      const errors = validateInputs("2.0", "0.5");
      expect(errors).toHaveLength(0);
    });
  });
});
